...
func Receive(name string, reader io.Reader, opts ReceiveOpts) (*ReceiveResult, error) {
	cmd := &Cmd{}
	r, w, err := os.Pipe()
	if err != nil {
		return nil, err
	...
	beginRecord := make([]byte, 312)

	_, err = io.ReadFull(reader, beginRecord)
	if err != nil {
		return nil, err
	}

	opts.BeginRecord = beginRecord
